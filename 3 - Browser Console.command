#!/bin/bash
# ===========================================================================
#  IGAOS - Solver Studio in a browser
#
...
# ===========================================================================
cd "$(dirname "$0")" || exit 1
printf '\033[1m  IGAOS - Solver Studio in a browser  \033[0m\n\n'
# ---------------------------------------------------------------------------
...
echo "Solver Studio: the same console, with a mouse."
echo "A browser tab will open at http://127.0.0.1:8420"
echo
echo "  Pick a model on the left, press Run solver, and watch the flow."
echo "  Untick cut separation and run again: same objective, sixty-one nodes"
echo "  instead of one. That difference is the demonstration."
echo
echo "Leave this window open while you demo. Control-C here stops the server."
echo
build_if_needed
command -v python3 >/dev/null 2>&1 || {
    echo "python3 is needed for the browser console and was not found."
    echo "The terminal console needs nothing extra - try 2 - Interactive Console.command"
    hold 1
}
echo
